    ...
    return (
        <div className="py-4">
            <div className="w-full mx-auto flex flex-col gap-3">
                <AuthWrapper>
                    <Card className="w-full shadow-sm border-none bg-card px-3 flex flex-row items-center gap-2 sm:gap-4 sticky top-0 z-40">
                        <CustomUserAvatar src={user?.profileImage} name={user?.name} size="md" />
                        <Button
                            variant="secondary"
                            className="flex-1 justify-start text-muted-foreground rounded-full h-10 px-4 sm:px-6 text-sm sm:text-base font-normal border-none bg-muted/50"
                            onClick={() => setPostModal(true)}
                        >
                            <span className="truncate">What is your question?</span>
                        </Button>
                        <Button
                            className="rounded-full px-4 sm:px-6 h-10 font-medium shadow-none bg-black/80"
                            onClick={() => setPostModal(true)}
                        >
                            Post
                        </Button>
